...
var _ Strategy = &FishStrategy{}

// NewFishStrategy returns the proper config for the FishStrategy
func NewFishStrategy() *StrategyCfg {
	return &StrategyCfg{
		Type: (&FishStrategy{}).GetType(),
	}
}

// FishStrategy is an implementation of Strategy.
...
func (nrs *FishStrategy) GetType() string {
	return "fish"
}

// UnrelayedSequencesOrdered returns the unrelayed sequence numbers between two chains
func (nrs *FishStrategy) UnrelayedSequencesOrdered(src, dst *Chain, sh *SyncHeaders) (*RelaySequences, error) {
...
func (nrs *FishStrategy) UnrelayedSequencesUnordered(src, dst *Chain, sh *SyncHeaders) (*RelaySequences, error) {
	return UnrelayedSequences(src, dst, sh)
}

// HandleEvents defines how the relayer will handle block and transaction events as they are emmited
func (nrs *FishStrategy) HandleEvents(src, dst *Chain, sh *SyncHeaders, events map[string][]string) {
	rlyPackets, err := fishRelayPacketsFromEventListener(src.PathEnd, dst.PathEnd, events)
	if len(rlyPackets) > 0 && err == nil {
		nrs.sendTxFromEventPackets(src, dst, rlyPackets, sh)
	}
}

func fishRelayPacketsFromEventListener(src, dst *PathEnd, events map[string][]string) (rlyPkts []relayPacket, err error) {
...
func (nrs *FishStrategy) sendTxFromEventPackets(src, dst *Chain, rlyPackets []relayPacket, sh *SyncHeaders) {
...
func (nrs *FishStrategy) RelayPacketsUnorderedChan(src, dst *Chain, sp *RelaySequences, sh *SyncHeaders) error {
...
func (nrs *FishStrategy) RelayPacketsOrderedChan(src, dst *Chain, sp *RelaySequences, sh *SyncHeaders) error {
	...
	for _, seq := range sp.Src {
		// Don't double requests..
		if skipTo != 0 && seq < skipTo {
			continue
		}

		chain, msg, newSeq, err := fishPacketMsgFromTxQuery(src, dst, sh, seq)
		if err != nil {
			return err
		}
		if seq != newSeq {
	...
	for _, seq := range sp.Dst {
		...
		chain, msg, newSeq, err := fishPacketMsgFromTxQuery(dst, src, sh, seq)
		if err != nil {
			return err
		}
		if seq != newSeq {
...
func fishPacketMsgFromTxQuery(src, dst *Chain, sh *SyncHeaders, seq uint64) (*Chain, []sdk.Msg, uint64, error) {
	...
	case err != nil:
		return nil, nil, seq, err
	case tx.Count == 0:
		return nil, nil, seq, fmt.Errorf("no transactions returned with query")
	case tx.Count > 1:
		return nil, nil, seq, fmt.Errorf("more than one transaction returned with query")
	}

	rcvPackets, timeoutPackets, err := fishRelayPacketFromQueryResponse(src.PathEnd, dst.PathEnd, tx.Txs[0], sh)
	switch {
	case err != nil:
		return nil, nil, seq, err
	case len(rcvPackets) == 0 && len(timeoutPackets) == 0:
		return nil, nil, seq, fmt.Errorf("no relay msgs created from query response")
		// case len(rcvPackets)+len(timeoutPackets) > 1:
		//	return nil, nil, seq, fmt.Errorf("more than one relay msg found in tx query")
	}
	...
	if len(rcvPackets) >= 1 {
		...
		for _, rp := range rcvPackets {

			// add all the packets and increase seq
			if err = rp.FetchCommitResponse(dst, src, sh); err != nil {
				return dst, nil, seq, err
			}
			msgs = append(msgs, rp.Msg(dst, src))
			seq++
		}

		// return the sending msg
		return dst, msgs, seq, nil
	}
	...
	if len(timeoutPackets) >= 1 {
		...
		for _, tp := range rcvPackets {
			// add all the packets and increase seq
			// fetch the timeout proof from the receiving chain
			if err = tp.FetchCommitResponse(src, dst, sh); err != nil {
				return nil, nil, seq, err
			}

			msgs = append(msgs, tp.Msg(src, dst))
			seq++

		}

		return src, msgs, seq, nil

	}

	// return the sending msg
	return src, nil, seq, nil
...
func fishRelayPacketFromQueryResponse(src, dst *PathEnd, res sdk.TxResponse, sh *SyncHeaders) (rcvPackets []relayPacket, timeoutPackets []relayPacket, err error) {
	for _, l := range res.Logs {
		for _, e := range l.Events {
			if e.Type == "send_packet" {
				...
				switch {
				case sh.GetHeight(dst.ChainID) >= rp.timeout:
					fmt.Println("Packet Expired! Dst Height / Timout", sh.GetHeight(dst.ChainID), rp.timeout)
					timeoutPackets = append(timeoutPackets, rp.timeoutPacket())
				case rp.timeoutStamp != 0 && time.Now().UnixNano() >= int64(rp.timeoutStamp):
					fmt.Println("Packet Expired! Expired at ", int64(rp.timeoutStamp))
